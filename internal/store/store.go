...
type Data struct { Guilds map[string]*GuildConfig `json:"guilds"` }
...
func DefaultConfig() *GuildConfig {
    return &GuildConfig{Enabled:true, Chance:2.5, CooldownSeconds:90, Mode:"normal", SummonWords:engine.DefaultSummonWords()}
}
...
func Open(path string) (*Store, error) {
    ...
    if s.data.Guilds == nil { s.data.Guilds = map[string]*GuildConfig{} }
    return s, nil
}
...
func (s *Store) Get(guildID string) GuildConfig {
    ...
    if !ok {
        c = DefaultConfig(); s.data.Guilds[guildID] = c
        _ = s.saveLocked()
    }
    cp := *c
...
func (s *Store) Update(guildID string, fn func(*GuildConfig)) error {
    s.mu.Lock(); defer s.mu.Unlock()
    c, ok := s.data.Guilds[guildID]
    if !ok { c = DefaultConfig(); s.data.Guilds[guildID] = c }
    fn(c)
    return s.saveLocked()
}
